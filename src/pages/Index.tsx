...
import { DashboardData, AppSettings, ProductGoal, Currency, CURRENCY_SYMBOLS, ProductCost, ProfitSettings } from '../types';
...
const Index = () => {
  ...
  const handleFileUpload = useCallback(async (event: React.ChangeEvent<HTMLInputElement>, type: 'bulk' | 'business') => {
    ...
    try {
      if (type === 'bulk') {
        ...
        setPendingData(prev => ({ ...prev, ...parsed }));
        setBulkFileUploaded(true);
        if (parsed.detectedCurrency) {
          setSettings(prev => ({
            ...prev,
            currencyCode: parsed.detectedCurrency!,
            currencySymbol: CURRENCY_SYMBOLS[parsed.detectedCurrency!] || '$',
          }));
        }
      } else {
        const businessReport = await processBusinessReport(file);
        setPendingData(prev => ({ ...prev, businessReport }));
  ...
  const handleLoadDemo = useCallback(() => {
    ...
    setTimeout(() => {
      ...
      setProductCosts(mock.mockCosts);
      setProductGoals(mock.mockGoals);
      setIsLoading(false);
    }, 500);
  }, []);
  ...
  const renderView = () => {
    ...
    switch (currentView) {
      case 'executive':
        return <ExecutiveDashboard data={dashboardData} previousData={previousData} settings={settings} />;
      case 'portfolio':
        return <PortfolioDashboard data={dashboardData} currencySymbol={currencySymbol} />;
      case 'asin-audit':
        return <AsinAuditDashboard data={dashboardData} previousData={previousData} currencySymbol={currencySymbol} settings={settings} />;
      case 'sp':
        return <SPDashboard data={dashboardData} currencySymbol={currencySymbol} />;
      case 'sb':
        return <SBDashboard data={dashboardData} currencySymbol={currencySymbol} />;
      case 'sd':
        return <SDDashboard data={dashboardData} currencySymbol={currencySymbol} />;
      case 'search-terms':
        return <SearchTermDashboard data={dashboardData} previousData={previousData} targetType="SP" currencySymbol={currencySymbol} settings={settings} />;
      case 'sb-search-terms':
        return <SearchTermDashboard data={dashboardData} previousData={previousData} targetType="SB" currencySymbol={currencySymbol} settings={settings} />;
      case 'diagnostics':
      ...
      case 'profit':
        return <ProfitDashboard data={dashboardData} currencySymbol={currencySymbol} costs={productCosts} profitSettings={profitSettings} onUpdateCosts={setProductCosts} onUpdateProfitSettings={setProfitSettings} productGoals={productGoals} settings={settings} />;
      case 'dayparting':
        return <DaypartingDashboard data={dashboardData} currencySymbol={currencySymbol} />;
      case 'forecasting':
        return <ForecastingDashboard data={dashboardData} currencySymbol={currencySymbol} settings={settings} />;
      case 'cannibalization':
        return <CannibalizationDashboard data={dashboardData} currencySymbol={currencySymbol} />;
      default:
        return <ExecutiveDashboard data={dashboardData} previousData={previousData} settings={settings} />;
    }
  };
        ...
        <div className="relative z-10 max-w-2xl w-full text-center">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-8 animate-fade-in" style={{ animationDelay: '0.3s' }}>
            <div className="relative">
              <label className={`group flex flex-col items-center justify-center p-8 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-300 ${bulkFileUploaded ? 'border-primary bg-primary/10' : isLoading ? 'opacity-50 cursor-wait' : 'border-border hover:border-primary hover:bg-primary/5 hover:shadow-[0_0_60px_-15px_hsl(var(--primary)/0.3)]'}`}>
                <input type="file" accept=".xlsx,.xls" onChange={e => handleFileUpload(e, 'bulk')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" disabled={isLoading} />
              ...
              {bulkFileUploaded && (
                <button onClick={(e) => { e.stopPropagation(); handleRemoveFile('bulk'); }} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-destructive/20 hover:bg-destructive/40 flex items-center justify-center text-destructive transition-all z-20" title="Remove file">
                  <X size={14} />
                </button>
              )}
            </div>
            ...
            <div className="relative">
              <label className={`group flex flex-col items-center justify-center p-8 rounded-2xl border-2 border-dashed cursor-pointer transition-all duration-300 ${businessFileUploaded ? 'border-primary bg-primary/10' : isLoading ? 'opacity-50 cursor-wait' : 'border-border hover:border-primary hover:bg-primary/5 hover:shadow-[0_0_60px_-15px_hsl(var(--primary)/0.3)]'}`}>
                <input type="file" accept=".xlsx,.xls,.csv" onChange={e => handleFileUpload(e, 'business')} className="absolute inset-0 w-full h-full opacity-0 cursor-pointer z-10" disabled={isLoading} />
              ...
              {businessFileUploaded && (
                <button onClick={(e) => { e.stopPropagation(); handleRemoveFile('business'); }} className="absolute top-2 right-2 w-7 h-7 rounded-full bg-destructive/20 hover:bg-destructive/40 flex items-center justify-center text-destructive transition-all z-20" title="Remove file">
                  <X size={14} />
                </button>
              )}
            </div>
          ...
          {bulkFileUploaded && (
            <div className="mb-8 animate-fade-in">
              <button onClick={handleProceedToDashboard} disabled={isLoading} className="flex items-center justify-center gap-3 w-full px-8 py-5 bg-primary text-primary-foreground font-heading font-bold text-lg rounded-2xl btn-glow transition-all duration-300 hover:scale-[1.02] disabled:opacity-50 disabled:hover:scale-100">
                <Zap size={20} /> {businessFileUploaded ? 'View Dashboard' : 'Continue with Bulk File Only'}
              </button>
              {!businessFileUploaded && (
                <p className="text-xs text-muted-foreground mt-3">You can also add the Business Report for enhanced analytics</p>
              )}
            </div>
          )}
