                ...
                <>
                    ...
                    <div className="space-y-4">
                        ...
                        <DataTable 
                            ...
                            columns={[
                                { key: 'term', header: 'Search Term', sortable: true, render: (r: TermOverlap) => <span className="font-bold text-slate-800 dark:text-zinc-200 text-sm">{r.term}</span> },
                                { key: 'autoCount', header: 'AUTO/PAT', align: 'center', render: (r: TermOverlap) => renderMatchBadge(r.autoCount, r.autoCount > 1 ? 'bg-orange-50 border-orange-200 text-orange-700 dark:bg-orange-900/30 dark:border-orange-800 dark:text-orange-300' : 'bg-slate-100 border-slate-200 text-slate-600 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-400') },
                                { key: 'exactCount', header: 'EXACT', align: 'center', render: (r: TermOverlap) => renderMatchBadge(r.exactCount, r.exactCount > 1 ? 'bg-rose-50 border-rose-200 text-rose-700 dark:bg-rose-900/30 dark:border-rose-800 dark:text-rose-300' : 'bg-brand-50 border-brand-200 text-brand-700 dark:bg-brand-900/30 dark:border-brand-800 dark:text-brand-300') },
                                ...
                                { key: 'acos', header: 'ACOS', align: 'right', sortable: true, render: (r: TermOverlap) => <span className={safeDiv(r.totalSpend, r.totalSales) > 0.4 ? 'text-rose-600 font-bold' : ''}>{formatPct(safeDiv(r.totalSpend, r.totalSales))}</span> },
                                { key: 'wastedSpend', header: 'Est. Waste', align: 'right', sortable: true, render: (r: TermOverlap) => <span className="font-bold text-rose-500">{formatCurrency(r.wastedSpend, currencySymbol)}</span> },
                            ]}
                            renderSubComponent={(row: TermOverlap) => (
                                <div className="bg-slate-50 dark:bg-zinc-900/50 rounded-lg p-4 border border-slate-200 dark:border-zinc-700 animate-fadeIn">
                                    <div className="flex items-center gap-2 mb-3">
                                        <ArrowUpRight className="text-slate-400" size={16} />
                                        <h5 className="text-xs font-bold uppercase tracking-widest text-slate-500">Campaign Breakdown for "{row.term}"</h5>
                                    </div>
                                    <div className="overflow-x-auto">
                                        <table className="w-full text-xs">
                                            <thead>
                                                <tr className="text-slate-400 dark:text-zinc-500 border-b border-slate-200 dark:border-zinc-700">
                                                    <th className="text-left py-2 font-bold uppercase pl-2">Campaign Name</th>
                                                    <th className="text-left py-2 font-bold uppercase">Match</th>
                                                    <th className="text-right py-2 font-bold uppercase">Sales</th>
                                                    <th className="text-right py-2 font-bold uppercase">% Sales</th>
                                                    <th className="text-right py-2 font-bold uppercase">Spend</th>
                                                    <th className="text-right py-2 font-bold uppercase">% Spend</th>
                                                    <th className="text-right py-2 font-bold uppercase">Orders</th>
                                                    <th className="text-right py-2 font-bold uppercase">ACOS</th>
                                                    <th className="text-right py-2 font-bold uppercase pr-2">Rec. Action</th>
                                                </tr>
                                            </thead>
                                            <tbody className="divide-y divide-slate-200 dark:divide-zinc-700">
                                                {row.conflicts.map((c, idx) => (
                                                    <tr key={idx} className={`group ${c.role === 'Primary' ? 'bg-emerald-50/50 dark:bg-emerald-900/10' : 'hover:bg-white dark:hover:bg-zinc-800'}`}>
                                                        <td className="py-2.5 pl-2 font-medium text-slate-800 dark:text-zinc-200">
                                                            <div className="flex items-center gap-2">
                                                                <span className="truncate max-w-[180px]" title={c.campaignName}>{c.campaignName}</span>
                                                                {c.role === 'Primary' && <span className="bg-emerald-100 text-emerald-700 text-[9px] px-1.5 rounded-sm font-bold flex items-center gap-1"><ArrowUpRight size={8}/> WINNER</span>}
                                                            </div>
                                                        </td>
                                                        <td className="py-2.5"><span className={`px-2 py-0.5 rounded text-[10px] font-mono ${c.matchType === 'EXACT' ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>{c.matchType}</span></td>
                                                        <td className="py-2.5 text-right font-medium">{formatCurrency(c.sales, currencySymbol)}</td>
                                                        <td className="py-2.5 text-right text-slate-500 font-mono">{formatPct(safeDiv(c.sales, row.totalSales))}</td>
                                                        <td className="py-2.5 text-right">{formatCurrency(c.spend, currencySymbol)}</td>
                                                        <td className="py-2.5 text-right text-slate-500 font-mono">{formatPct(safeDiv(c.spend, row.totalSpend))}</td>
                                                        <td className="py-2.5 text-right">{c.orders}</td>
                                                        <td className={`py-2.5 text-right font-bold ${c.acos > 0.4 ? 'text-rose-500' : c.acos < 0.2 && c.sales > 0 ? 'text-emerald-500' : 'text-slate-600'}`}>{formatPct(c.acos)}</td>
                                                        <td className="py-2.5 text-right pr-2">
                                                            {c.role === 'Primary' ? (
                                                                <span className="text-emerald-600 font-bold text-[10px] uppercase tracking-wider">Maintain</span>
                                                            ) : (
                                                                <span className={`text-[10px] font-bold uppercase tracking-wider ${c.acos > 0.5 ? 'text-rose-600' : 'text-amber-600'}`}>
                                                                    {c.acos > 0.5 ? 'Reduce Bid' : c.matchType === 'EXACT' ? 'Pause' : 'Negate'}
                                                                </span>
                                                            )}
                                                        </td>
                                                    </tr>
